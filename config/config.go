...
func setEnvVars(conf *Config, tests bool) error {
    ...
    conf.LocalDir = os.Getenv("LOCAL_DIR")
    conf.LocalBackupFile = os.Getenv("LOCAL_BACKUP_FILE")
    localdryrun := os.Getenv("LOCAL_DRY_RUN")
    if (localdryrun != "") {
        conf.LocalDryRunRestore = true
    }
    ...
    if acceptanceTest == "" {
        ...
        if tests {
            ...
            log.Printf("Using LocalDir %s skipping ENV var requirements\n", conf.LocalDir)
        }
    } else {

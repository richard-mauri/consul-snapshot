...
func doWork(conf *config.Config, client *consul.Consul) error {
    ...
    } else if (conf.LocalDir != "") {
        log.Printf("[INFO] Local backup file: %s\n", b.FullFilename)
        log.Print("[INFO] Skipping remote backup")

        // Remove the staging path
        err = os.RemoveAll(b.LocalFilePath)
        if err != nil {
            log.Printf("Unable to remove temporary backup: %s : %v", b.LocalFilePath, err)
        }
    } else {
...
func (b *Backup) preProcess() {
    ...
    var localDir string
    if (b.Config.LocalDir != "") {
        localDir = b.Config.LocalDir
    } else {
        localDir = b.Config.TmpDir
    }
    dir := filepath.Join(localDir, prefix)
    err := os.MkdirAll(dir, 0777)
    if err != nil {
        log.Fatalf("Unable to create tmpdir %s: %v", localDir, err)
    }
...
func (b *Backup) compressStagedBackup() {
    ...
    finalpath := filepath.Join(localDir, finalfile)
